...
$$
Où $\theta$ est la valeur de l'option à un noeuds quelconque.
...
Donc étant donné que le delta est négatif, pour répliquer l'option de vente, il faut donc se procurer une position courte dans le sous-jacent. Pour y arriver, il faut prêter un montant  B$\$$.
...
On peut en déduire que plus le temps est grand avant la date d'échéance, plus les probabilités qu'il y ait une valeur à l'échéance non nulle d'une option achat sont grandes. C'est notament le cas en raison du taux sans risque plus grand que le taux de dividende ($r_f$ $\ge$ $\delta$), donc l'espérance de la hausse de prix du sous-jacent est positive au fil du temps. Les prix pour les options d'achat sont présentés dans le tableau suivant.
...
À l'invserse des options d'achat, on peut en déduire que plus le temps est grand avant la date d'échéance, plus les probabilités qu'il y ait une valeur à l'échéance non nulle d'une option achat sont petites. Les prix pour les options de vente sont présentés dans le tableau suivant. 
...
Maintenant, les mêmes caractérisques sont maintenues, mais au lieu de faire le modèle à 4 périodes, il y aura 100 périodes. Le prix des options se rapprochent de plus en plus du prix calculé à l'aide du modèle continue de *Black-Scholes* qui sera présenté plus loin dans le rapport. Le tableau ci-dessous présente les prix initiaux pour les options d'achat ainsi que la composition initiale du portefeuille réplicatif.
...
Le tableau ci-dessous présente les prix initiaux pour les options de vente pour le modèle à 100 périodes. Comme pour les options d'achat, il sera démontré plus loin à la question 5 que le prix d'une option de vente dans un modèle binomial à 100 périodes tend vers le prix prédit par le modèle *Black-Scholes*. De plus, la composition initiale du portefeuille réplicatif est présentée.
